...
/**
 * ZooKeeper中客户端和服务端通过网络通信,其本质是通过Java的IO数据流的方式进行通信,
 * 但是传统的IO方式具有阻塞等待的问题,而NIO框架作为传统的Java IO框架的替代方案,
 * 在性能上大大优于前者.也正因如此,NIO 框架也被广泛应用于网络传输的解决方案中.而ZooKeeper
 * 最早也是使用自己实现的NIO框架,但是从3.4.0 版本后,引入了第三方Netty等框架来满足不同
 * 使用情况的需求,而我们可以通过ServerCnxnFactory类来设置ZooKeeper服务器,从而在运行的
 * 时候使用我们指定的NIO框架
 */
