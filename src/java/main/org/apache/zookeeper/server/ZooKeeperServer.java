...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    public synchronized void startup() {
        if (sessionTracker == null) {
             ...
             * 在ZooKeeper中,会话的超时异常包括客户端readtimeout异常和服务器端sessionTimeout异常.
             * 在我们平时的开发中,要明确这两个异常的不同之处在于一个是发生在客户端,而另一个是发生在服务端.
             *
             * 而对于那些对ZooKeeper接触不深的开发人员来说,他们常常踩坑的地方在于,虽然设置了超时间,
             * 但是在实际服务运行的时候ZooKeeper并没有按照设置的超时时间来管理会话.
             *
             * 这是因为ZooKeeper实际起作用的超时时间是通过客户端和服务端协商决定.
             * ZooKeeper 客户端在和服务端建立连接的时候,会提交一个客户端设置的会话超时时间,
             * 而该超时时间会和服务端设置的最大超时时间和最小超时时间进行比对,如果正好在其允许的范围内,
             * 则采用客户端的超时时间管理会话.如果大于或者小于服务端设置的超时时间,则采用服务端设置的值管理会话.
             */
